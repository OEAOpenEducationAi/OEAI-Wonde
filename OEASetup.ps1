...
# Get all .ipynb files in the folder
$pipelineFiles = Get-ChildItem -Path $pipelinesFolder -Filter *.json

# Loop through each notebook file
foreach ($pipelineFile in $pipelineFiles) {
    # Read the content of the notebook file
    $pipelineContent = Get-Content -Path $pipelineFile.FullName -Raw

    # Convert the content to Base64
    $base64Content = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($pipelineContent))


    # Display the inline Base64 string or perform any other actions as needed
    Write-Output "Notebook $($pipelineFile.Name) converted to inline Base64 string:"
    
	$pipelineName = $pipelineFile.Name

	# Find the position of the first dot
	$dotPosition = $pipelineName.IndexOf('.')

	# Check if a dot was found
	if ($dotPosition -ge 0) {
		# Trim the string up to the first dot
		$pipelineName = $pipelineName.Substring(0, $dotPosition)
	...
	$pipelineInstance = $existingItems | Where-Object { $_.DisplayName -eq $pipelineName }
	
	if($pipelineInstance){Write-Host "already present $($pipelineName)"}
	else
	{
		# Construct the JSON string using double-quoted string to expand $name
		$jsonBody = @"
